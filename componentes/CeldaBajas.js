...
export default class CeldaBajas extends Component {
  constructor(props) {
    super(props);
    this.state = {
    ...
    const Modifica = () => {
      ...
      xhttpD.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
          ...
          if (xhttpD.responseText == '1') {
            Alert.alert('Tu cita se modifico con exito');
            _this.setState({bandera: 0});
            _this.state.dia = fechaSeparada[2];
            _this.state.mes = fechaSeparada[1];
            _this.state.hora = horario;
            _this.setState({
              datosCita:
                'Hora:' +
                _this.state.hora +
      ...
      xhttpD.open(
        'GET',
        'https://josess.000webhostapp.com/EditCitas.php?&mes=' +
          fechaSeparada[1] +
          '&dia=' +
          fechaSeparada[2] +
          '&hora=' +
